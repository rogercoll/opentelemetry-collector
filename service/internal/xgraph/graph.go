...
// Package xgraph contains the internal xgraph representation of the pipelines.
//
...
package xgraph // import "go.opentelemetry.io/collector/service/internal/xgraph"
...
func (g *Graph) StartAll(ctx context.Context, host *Host) error {
	...
	for i := len(nodes) - 1; i >= 0; i-- {
		err := g.startNode(ctx, host, nodes[i])
		if err != nil {
			return err
		}
	}
...
func (g *Graph) startNode(ctx context.Context, host *Host, node graph.Node) error {
...
func (g *Graph) ShutdownAll(ctx context.Context, reporter status.Reporter) error {
	...
	for i := 0; i < len(nodes); i++ {
		compErr := g.shutdownNode(ctx, reporter, nodes[i])
		if compErr != nil {
...
func (g *Graph) shutdownNode(ctx context.Context, reporter status.Reporter, node graph.Node) error {
	comp, ok := node.(component.Component)
